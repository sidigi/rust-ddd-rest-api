...
impl<'a> Renderer<'a>{
    pub fn new(template_dir: std::path::PathBuf) -> Self {
        ...
        renderer.register_templates_directory(".hbs", &template_dir).expect("failed to register templates directory");
        Self(renderer)
    }

    fn convert_to_value<S: serde::Serialize+std::fmt::Debug>(serializable: &S) -> serde_json::Value {
    ...
    fn do_render(&self, path: &str, ctx: serde_json::Value) -> String {
        self.0.render(path, &ctx).expect("failed to render template")
    }
}